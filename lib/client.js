window.__ModuleLoader__.load({
	id: "dsh-codex-subscription-sssv4",
	factory: (require) => {
		var module = { exports: {} };
...
`;
		...
		const zh = {
			...
			usagePrevious: "上次成功的额度，可能已变化。",
			usagePreviousRefreshing: "正在刷新；下方保留上次成功的额度。",
			usagePreviousAfterError: "刷新失败；下方仍显示上次成功的额度。",
		...
		const en = {
			...
			usagePrevious: "Last successful quota; it may have changed.",
			usagePreviousRefreshing: "Refreshing; the last successful quota remains visible below.",
			usagePreviousAfterError: "Refresh failed; the last successful quota remains visible below.",
...
`;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						}), state?.available ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Button, {
							type: "button",
							variant: "outline",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", { children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: "@deepseek-ai/dsh-subagent-codex@0.1.5-rc.2" }),
		...
		*/
		...
		//#endregion
		//#region src/client-usage-cache.js
		const PREFIX = "dsh-codex-subscription/usage-cache-v1:";
		const MAX_AGE_MS = 10080 * 60 * 1e3;
		function quotaProjection(value) {
			if (!Number.isFinite(value?.fetchedAt) || !Array.isArray(value.rateLimits)) return void 0;
			const rateLimits = value.rateLimits.slice(0, 16).map((limit) => {
				if (typeof limit?.id !== "string" || !Array.isArray(limit.windows)) return void 0;
				const windows = limit.windows.slice(0, 8).map((window) => {
					if (!Number.isFinite(window?.remainingPercent) || window.remainingPercent < 0 || window.remainingPercent > 100 || !Number.isFinite(window.windowSeconds) || window.windowSeconds <= 0) return void 0;
					return {
				...
				if (windows.some((window) => window === void 0)) return void 0;
				return {
					id: limit.id.slice(0, 80),
					...typeof limit.name === "string" ? { name: limit.name.slice(0, 120) } : {},
					windows
				};
			});
			if (rateLimits.some((limit) => limit === void 0) || rateLimits.length === 0) return void 0;
			return {
				fetchedAt: value.fetchedAt,
				rateLimits
			};
		}
		function createUsageCache({ storage = () => globalThis.localStorage, now = Date.now } = {}) {
			...
			const freshEnough = (value) => value !== void 0 && now() - value.fetchedAt >= 0 && now() - value.fetchedAt <= MAX_AGE_MS;
			return Object.freeze({
				read(accountKey) {
					if (typeof accountKey !== "string" || accountKey.length === 0) return void 0;
					let value = memory.get(accountKey);
					if (value === void 0) try {
						value = quotaProjection(JSON.parse(backing()?.getItem(keyFor(accountKey)) ?? "null"));
					} catch {
						return;
					}
					if (!freshEnough(value)) {
				...
				write(accountKey, usage) {
					...
					memory.set(accountKey, value);
					try {
						backing()?.setItem(keyFor(accountKey), JSON.stringify(value));
					} catch {}
				},
				clear() {
					memory.clear();
				...
				prune(accountKeys) {
					...
					try {
						for (let index = target.length - 1; index >= 0; index -= 1) {
							const key = target.key(index);
							if (key?.startsWith(PREFIX) && !allowed.has(decodeURIComponent(key.slice(38)))) target.removeItem(key);
						}
					} catch {}
				}
			});
		...
		async function importPayload(file) {
			...
			let binary = "";
			for (let offset = 0; offset < bytes.length; offset += 32768) binary += String.fromCharCode(...bytes.subarray(offset, Math.min(bytes.length, offset + 32768)));
			return {
				name: file.name,
				encoded: btoa(binary)
			};
		...
		const quotaWindowLabel = (seconds) => {
			if (seconds >= 17100 && seconds <= 18900) return "5h";
			if (seconds >= 574560 && seconds <= 635040) return "周";
			if (seconds >= 82080 && seconds <= 90720) return "日";
			return seconds >= 86400 && seconds % 86400 === 0 ? `${seconds / 86400}天` : `${Math.round(seconds / 360) / 10}h`;
		};
		const weeklyQuotaPressure = (snapshot) => {
			const window = (snapshot?.usage?.rateLimits?.find((item) => item.id === "codex")?.windows ?? []).find((item) => item.windowSeconds >= 574560 && item.windowSeconds <= 635040);
			if (window === void 0 || !Number.isFinite(window.remainingPercent) || !Number.isSafeInteger(window.resetsAt)) return void 0;
			const hours = (window.resetsAt * 1e3 - Date.now()) / 36e5;
			if (!Number.isFinite(hours) || hours <= 0) return void 0;
			return window.remainingPercent / hours;
		};
		const quotaResetLabel = (resetsAt) => {
			if (!Number.isSafeInteger(resetsAt)) return void 0;
			const date = /* @__PURE__ */ new Date(resetsAt * 1e3);
			if (!Number.isFinite(date.getTime())) return void 0;
			const now = /* @__PURE__ */ new Date();
			const sameDay = date.getFullYear() === now.getFullYear() && date.getMonth() === now.getMonth() && date.getDate() === now.getDate();
			return date.toLocaleString(void 0, sameDay ? {
				hour: "2-digit",
				minute: "2-digit"
			} : {
				month: "numeric",
		...
		function AccountQuota({ snapshot }) {
			if (snapshot === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "codexSubscriptionAccountQuota codexSubscriptionAccountQuotaMuted",
				children: "额度读取中…"
			});
			if (snapshot.disabled) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "codexSubscriptionAccountQuota codexSubscriptionAccountQuotaMuted",
				children: "已停用，未查询额度"
			});
			if (snapshot.error && snapshot.usage === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "codexSubscriptionAccountQuota codexSubscriptionAccountQuotaError",
				children: snapshot.error === "ChatGPT sign-in needs to be renewed" ? "账号认证失效" : "额度读取失败"
			});
			const windows = (snapshot.usage?.rateLimits?.find((item) => item.id === "codex") ?? snapshot.usage?.rateLimits?.[0])?.windows ?? [];
			if (windows.length === 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "codexSubscriptionAccountQuota codexSubscriptionAccountQuotaMuted",
				children: "暂无额度数据"
			});
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				className: "codexSubscriptionAccountQuota",
				children: [windows.map((window, index) => {
					const reset = quotaResetLabel(window.resetsAt);
					const remaining = Number(window.remainingPercent).toLocaleString(void 0, { maximumFractionDigits: 1 });
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						className: "codexSubscriptionAccountQuotaWindow",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("strong", { children: [
							quotaWindowLabel(window.windowSeconds),
							" ",
							remaining,
							"%"
						] }), reset ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("small", { children: [reset, " 重置"] }) : null]
					}, `${window.windowSeconds}-${index}`);
				}), snapshot.error ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", {
					className: "codexSubscriptionAccountQuotaError",
					children: snapshot.error === "ChatGPT sign-in needs to be renewed" ? "认证失效 · 上次结果" : "刷新失败 · 上次结果"
				}) : snapshot.cached ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", {
					className: "codexSubscriptionAccountQuotaMuted",
					children: "上次结果"
				}) : null]
			});
		}
		...
		function AccountSchedulingControls({ candidate, busy, emailVisible, onConfigure, quotaPressure }) {
			...
			const [priorityDraft, setPriorityDraft] = (0, react.useState)(String(priority));
			const [weightDraft, setWeightDraft] = (0, react.useState)(String(weight));
			const accessibleName = candidate.email ? emailVisible ? candidate.email : maskEmail(candidate.email) : candidate.label;
			(0, react.useEffect)(() => setPriorityDraft(String(priority)), [candidate.id, priority]);
			(0, react.useEffect)(() => setWeightDraft(String(weight)), [candidate.id, weight]);
			const commit = (field, draft, original, minimum, maximum, reset) => {
				if (draft.trim() === "") {
					reset(String(original));
					return;
				}
				const value = Number(draft);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "codexSubscriptionAccountSchedule",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["优先级 ", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						type: "number",
						min: "-1000",
						max: "1000",
						step: "1",
						...
						onBlur: () => commit("priority", priorityDraft, priority, -1e3, 1e3, setPriorityDraft),
						onKeyDown: (event) => {
							if (event.key === "Enter") event.currentTarget.blur();
						}
					})] }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["权重 ", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						type: "number",
						min: "1",
						max: "100",
						step: "1",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						title: "周剩余额度 ÷ 距周重置时间",
						children: ["额度压力 ", Number.isFinite(quotaPressure) ? `${quotaPressure.toLocaleString(void 0, { maximumFractionDigits: 2 })}%/h` : "—"]
					})
				]
			});
		}
		function AccountCard({ rpc, t, account, setAccount, onSignedOut }) {
			...
			const [scheduler, setScheduler] = (0, react.useState)();
			const [importSummary, setImportSummary] = (0, react.useState)();
			const [accountUsage, setAccountUsage] = (0, react.useState)({});
			const [accountUsageError, setAccountUsageError] = (0, react.useState)(false);
			const visibleAccountUsage = Object.fromEntries(accounts.map((candidate) => {
				if (candidate.enabled === false) return [candidate.id, {
					id: candidate.id,
					disabled: true
				}];
				const previous = usageCache.read(candidate.id);
				return [candidate.id, accountUsage[candidate.id] ?? (previous ? {
					id: candidate.id,
					usage: previous,
					cached: true
				} : void 0)];
			...
			const quotaRequest = (0, react.useRef)(0);
			const importRef = (0, react.useRef)();
			...
			const loadAccountUsage = (force) => {
				...
				setQuotaBusy(true);
				setAccountUsageError(false);
				recoveryCall(rpc, "usage/accounts", { force }, 45e3).then((value) => {
					if (quotaRequest.current !== request) return;
					setAccountUsage((previous) => {
						const next = { ...previous };
						for (const item of value.accounts ?? []) if (item.usage) {
			...
			(0, react.useEffect)(() => {
				...
				let live = true;
				call("scheduler/status").then((value) => {
					if (live) setScheduler(value);
				}).catch(() => {});
				return () => {
					live = false;
				};
			...
			const configureAccount = (id, patch) => {
				setBusy(true);
				setError(void 0);
				call("account/configure", {
					id,
					...patch
				}).then((next) => {
					setAccount(next);
					return call("scheduler/status");
				}).then(setScheduler).catch((error) => setError(error instanceof Error ? error.message : t("failed"))).finally(() => setBusy(false));
			};
			const updateScheduler = (patch) => {
				setBusy(true);
				setError(void 0);
				call("scheduler/update", patch).then(async (next) => {
					setScheduler(next);
					if (Object.hasOwn(patch, "fixedAccountId")) {
						setAccount(await call("status"));
						loadAccountUsage(true);
						notifyQuickQuota();
					}
				}).catch((error) => setError(error instanceof Error ? error.message : t("failed"))).finally(() => setBusy(false));
			};
			const importAccounts = (event) => {
				...
				(async () => {
					...
					setImportSummary(`新增 ${added} 个，更新 ${updated} 个，跳过 ${duplicates} 个完全相同账号${total === void 0 ? "" : `，当前共 ${total} 个`}`);
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: signedIn ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Button, {
									type: "button",
									variant: "outline",
									disabled: quotaBusy || loginVisible,
									onClick: () => loadAccountUsage(true),
									children: quotaBusy ? "刷新中…" : "刷新额度"
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Button, {
									type: "button",
									variant: "outline",
					...
					signedIn && scheduler !== void 0 && accounts.length > 1 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "codexSubscriptionFlow",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "codexSubscriptionActions",
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["调度策略 ", /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
									disabled: busy || scheduler.config?.fixedAccountId !== void 0,
									value: scheduler.config?.strategy ?? "fill-first",
									onChange: (event) => updateScheduler({ strategy: event.currentTarget.value }),
									children: [
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
											value: "quota-balanced",
											children: "额度均衡"
										})
									]
								})] }),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["固定账号（测试） ", /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
									disabled: busy,
									value: scheduler.config?.fixedAccountId ?? "",
									onChange: (event) => updateScheduler({ fixedAccountId: event.currentTarget.value || null }),
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
										value: "",
										children: "关闭固定模式"
									}), accounts.filter((candidate) => candidate.enabled !== false).map((candidate) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
										value: candidate.id,
										children: candidate.email ? maskEmail(candidate.email) : candidate.label
									}, candidate.id))]
								})] }),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									type: "checkbox",
									disabled: busy || scheduler.config?.fixedAccountId !== void 0,
									checked: scheduler.config?.sessionAffinity !== false,
									onChange: (event) => updateScheduler({ sessionAffinity: event.currentTarget.checked })
								}), " 同一对话固定账号"] }),
								scheduler.config?.fixedAccountId === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "codexSubscriptionAccountQuota codexSubscriptionAccountQuotaMuted",
									children: "固定模式：所有对话只使用所选账号，不轮询、不自动切号"
								})
							]
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "codexSubscriptionPreferenceHint",
							children: "先选择优先级最高的已启用账号；同级账号再按所选策略调度。权重只用于加权轮询；额度均衡按“周剩余额度 ÷ 距周重置时间”动态选择；同一对话固定账号时，只在首次分配时选择。"
						})]
					}) : null,
					signedIn && scheduler !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "codexSubscriptionFlow",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							type: "checkbox",
							disabled: busy,
							checked: scheduler.config?.rollingActivation !== false,
							onChange: (event) => updateScheduler({ rollingActivation: event.currentTarget.checked })
						}), " Plus 5h 自动续活"] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							className: "codexSubscriptionPreferenceHint",
							children: "窗口到期后用独立零上下文的 Luna Low 极简请求续启下一轮 5h；不进入任何用户会话，也不继承 system/developer、工具或项目上下文。"
						})]
					}) : null,
					signedIn && failedAccounts.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
						className: "codexSubscriptionError",
						role: "status",
						children: [
							failedAccounts.length,
							" 个账号额度不可读：",
							failedAccounts.map((candidate) => candidate.email ? maskEmail(candidate.email) : candidate.label).join("、"),
							"。这不代表订阅到期；请先刷新额度，持续失败时检查登录凭据。"
						]
					}) : null,
					signedIn && accountUsageError ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: "codexSubscriptionPreferenceHint",
						role: "status",
						children: "逐账号额度本次刷新未完成，仍显示上次成功结果；可以稍后重试。"
					}) : null,
					signedIn && accounts.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: accounts.map((candidate) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "codexSubscriptionAccountCopy",
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: "codexSubscriptionAccountName",
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(AccountEmail, {
											...
											className: "codexSubscriptionAccountState",
											children: candidate.enabled === false ? "已停用" : "已启用"
										})]
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(AccountQuota, { snapshot: visibleAccountUsage[candidate.id] }),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(AccountSchedulingControls, {
										candidate,
										busy: busy || loginVisible,
										emailVisible: emailVisibleForAccount,
										onConfigure: configureAccount,
										quotaPressure: weeklyQuotaPressure(visibleAccountUsage[candidate.id])
									})
								]
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "codexSubscriptionActions",
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Button, {
										...
										variant: "outline",
										disabled: busy || loginVisible,
										onClick: () => configureAccount(candidate.id, { enabled: candidate.enabled === false }),
										children: candidate.enabled === false ? "启用" : "停用"
									}),
									candidate.active || candidate.enabled === false ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Button, {
										type: "button",
										variant: "outline",
										disabled: busy || loginVisible,
					...
					importSummary !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: "codexSubscriptionPreferenceHint",
						role: "status",
		...
		function UsageCard({ rpc, t, signedIn, accountKey, resetKey, preference }) {
			const [usage, setUsage] = (0, react.useState)(() => signedIn ? usageCache.read(accountKey) : void 0);
			const [cached, setCached] = (0, react.useState)(() => signedIn && usageCache.read(accountKey) !== void 0);
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							signedIn && usage !== void 0 && (busy || cached || error !== void 0) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: "codexSubscriptionCreditNote",
								role: "status",
								children: t(error !== void 0 ? "usagePreviousAfterError" : busy ? "usagePreviousRefreshing" : "usagePrevious")
							}) : null,
		...
		function CodexSection({ preference, rpc, accountStatus, t }) {
			...
			const activeAccountId = account?.accounts?.find((candidate) => candidate.active)?.id ?? (accountKeys.length === 0 ? "legacy" : void 0);
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							account === void 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(UsageCard, {
								...
								accountKey: activeAccountId,
								resetKey,
								preference
							}, `${activeAccountId ?? "unknown"}:${resetKey}`),
