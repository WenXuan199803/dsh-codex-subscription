...
async function recoverToolStep(exec, next, onRetry = () => {}) {
...
function registerToolStepRecovery(ctx) {
	return ctx.on?.("tools/execute", (exec, next) => recoverToolStep(exec, next, (name, attempt) => ctx.logger?.debug?.("Codex read-only tool %s retry %d", name, attempt)));
}
//#endregion
