...
function guardSseModel(response, target, requestedModel) {
...
function classifyFailure(failure = {}) {
	...
	if ([408].includes(status) || TRANSPORT_CODES.has(code) || /^(?:CERT_|ERR_TLS_|ERR_SSL_)/u.test(code) || /TIMEOUT|NETWORK|CONNECTION|TRANSPORT|SOCKET|STREAM_INCOMPLETE/u.test(code) || /timed? out|ECONN|EPIPE|EOF|network/iu.test(message)) return {
		retryable: true,
		scope: "transport",
		reason: "transient",
		cooldownMs: retryAfterMs ?? 10 * 1e3
