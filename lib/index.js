...
function proxyResponseBody(response, signal) {
	...
	return new ReadableStream({
		async pull(controller) {
			try {
				...
				controller.error(!signal?.aborted && reset ? new Error(`Codex response stream connection closed (${code})`, { cause: error }) : error);
				reader.releaseLock();
			}
		},
		async cancel(reason) {
...
const PACKAGE_VERSION = "2.1.0-beta.5.sssv4.1";
