...
import { TOOL_RUNTIME_SCHEDULER, defineTool } from "@deepseek-ai/dsh-tools";
...
const ACCOUNT_ROLLING_ACTIVATION_FIELD = "accountRollingActivation";
...
//#endregion
//#region src/model-integrity.js
/** Validate the upstream model before the buffered adapter exposes any output. */
function guardSseModel(response, target, requestedModel, onFrame = () => {}) {
	if (target.hostname !== "chatgpt.com" || target.pathname !== "/backend-api/codex/responses" || !response.ok || !response.body || typeof requestedModel !== "string") return response;
	...
	const validateFrame = (frame) => {
		for (const line of frame.split(/\r?\n/u)) {
			...
			if (typeof actual === "string" && actual.length > 0 && actual !== requestedModel) throw Object.assign(/* @__PURE__ */ new Error(`Upstream model mismatch: requested ${requestedModel}, received ${actual}`), { code: "CODEX_MODEL_MISMATCH" });
		}
	};
...
function guardWebSocketModel(socket, requestedModel) {
	...
	socket.addEventListener = (type, listener, options) => {
		...
		const guarded = (event) => {
			...
			if ([
				...
				"response.completed",
				"response.done"
			].includes(frame?.type) && typeof frame.response?.model === "string" && frame.response.model.length > 0 && frame.response.model !== requestedModel) {
				rejected = true;
...
async function withCodexNetwork(run, options = {}) {
	if (options.websocket && activeWebSocketScopes === 0) {
		...
		scopedWebSocket = new Proxy(baseWebSocket ?? WebSocket, { construct(target, args, newTarget) {
			...
			return guardWebSocketModel(scope.options.createWebSocket?.(args[0], args[1]) ?? new WebSocket(args[0], {
				...args[1],
				...proxy ? { agent: new HttpsProxyAgent(proxy) } : {}
			}), scope.options.expectedModel);
		} });
...
function classifyFailure(failure = {}) {
	...
	].includes(code) || /context (?:window|length).{0,60}(?:exceed|too large)|too many tokens/iu.test(message)) return {
		retryable: false,
		scope: "request",
		reason: "error",
		cooldownMs: 0
	...
	if (/OVERLOAD|CAPACITY/u.test(code) || /server_is_overloaded|selected model is at capacity|model_at_capacity/iu.test(message)) return {
		retryable: true,
		scope: "provider",
		reason: "transient",
	...
	if ([
		...
		"MODEL_UNAVAILABLE",
		"MODEL_ACCESS_DENIED"
	].includes(code) || ["CODEX_MODEL_MISMATCH", "CODEX_IMAGE_INCOMPLETE"].includes(code) || /upstream model mismatch/iu.test(message) || /model.{0,100}(?:not available|not found|not supported|does not exist).{0,100}(?:this account|your account|credential)/iu.test(message) || /(?:do not have access|don't have access).{0,100}model/iu.test(message) || (status === 400 || status === 404) && /model.{0,80}(?:not found|not available|not supported|does not exist|do not have access)/iu.test(message)) return {
		retryable: true,
		scope: "model",
		reason: "model-access",
		cooldownMs: 60 * 1e3
	};
	if ([401, 403].includes(status) || /AUTH|UNAUTHORIZED|FORBIDDEN/.test(code) || /invalidated oauth token|invalid(?:ated)? oauth access token|sign-in needs to be renewed|subscription authorization failed/iu.test(message)) return {
		retryable: true,
		scope: "credential",
		reason: "auth",
		cooldownMs: 60 * 1e3
	};
	if ([408].includes(status) || TRANSPORT_CODES.has(code) || /^(?:CERT_|ERR_TLS_|ERR_SSL_)/u.test(code) || /TIMEOUT|NETWORK|CONNECTION|TRANSPORT|SOCKET|STREAM_INCOMPLETE/u.test(code) || /timeout|timed? out|ECONN|EPIPE|EOF|network/iu.test(message)) return {
		retryable: true,
		scope: "transport",
		reason: "transient",
		cooldownMs: retryAfterMs ?? 10 * 1e3
	...
	if ([
		...
		525,
		526
	].includes(status) || /SERVICE_UNAVAILABLE|UPSTREAM/u.test(code) || /service unavailable/iu.test(message)) return {
		retryable: true,
		scope: "provider",
		reason: "transient",
		cooldownMs: retryAfterMs ?? 10 * 1e3
...
function failureFromError(error) {
	...
	const status = Number(error?.status) || Number(/\bHTTP (\d{3})\b/iu.exec(message)?.[1]) || void 0;
	const causes = [];
...
var CodexAccountScheduler = class {
	...
	async choose(sessionId, excluded = /* @__PURE__ */ new Set(), model) {
		...
		if (typeof config.fixedAccountId === "string" && config.fixedAccountId.length > 0) return accounts.find((account) => account.id === config.fixedAccountId && account.enabled !== false && !excluded.has(account.id));
		const enabled = accounts.filter((account) => account.enabled !== false && !excluded.has(account.id) && !this.cooldowns.has(account.id) && !(model && this.cooldowns.has(`${account.id}\u0000${model}`)));
		...
		if (config.strategy === "quota-balanced") {
			...
			if (complete) for (const account of pool) {
				let pressure;
				try {
					pressure = this.resolveQuotaPressure(account.id, now);
				} catch {
					pressure = void 0;
				}
				...
				if (best === void 0 || pressure > best.pressure) best = {
					account,
					pressure
				};
			}
			selected = complete && best !== void 0 ? best.account : pool[0];
...
async function runScheduledAccountOperation({ scheduler, store, sessionId, model, signal, operation }) {
	if ((await scheduler.accounts()).length === 0) throw new LlmError("没有可用的 Codex 账号", "CODEX_ACCOUNT_POOL_UNAVAILABLE");
	const excluded = /* @__PURE__ */ new Set(), attempts = [];
	let lastError, round = 0, transientInRound = false;
	const deadline = Date.now() + 3e4;
	while (true) {
		signal?.throwIfAborted();
		if (!(await scheduler.accounts()).some((account) => account.enabled !== false && !excluded.has(account.id))) {
			const delay = Math.min(3e3, 150 * 2 ** Math.min(round, 5));
...
var ScheduledCodexAdapter = class extends LlmAdapter {
	...
	async *run(options, dispatch) {
		if ((await this.scheduler.accounts()).length === 0) throw new LlmError("没有可用的 Codex 账号：请先登录或导入账号", "CODEX_ACCOUNT_POOL_UNAVAILABLE");
		...
		while (true) {
			...
			if (!(await this.scheduler.accounts()).some((account) => account.enabled !== false && !excluded.has(account.id))) {
				const delay = Math.min(3e3, 150 * 2 ** Math.min(round, 5));
				if (!transientInRound || Date.now() + delay > deadline) break;
				round++;
			...
			try {
				...
				for await (const chunk of scopedIterator(this.store, account.id, iterable)) {
					if (chunk?.type === "finish" && ["error", "aborted"].includes(chunk.reason?.kind)) {
						if (options.signal?.aborted) throw options.signal.reason ?? /* @__PURE__ */ new Error("Request aborted");
				...
				const failure = {
					...
					message: "Codex stream ended before finish"
				};
...
const SUBAGENT_RUNTIME_VERSION = "0.1.5-rc.3";
const SUPPORTED_RUNTIME_VERSIONS = /* @__PURE__ */ new Set(["0.1.5-rc.2", SUBAGENT_RUNTIME_VERSION]);
...
function inspectSubagentRuntime(resolve = require.resolve) {
	try {
		...
		if (!SUPPORTED_RUNTIME_VERSIONS.has(manifest.version)) return {
			installed: false,
			present: true
		};
...
function createSubscriptionSubagent({ ctx, nativeHome, resolveAuth, store, scheduler, refresh, loadRuntime, maintenance = () => false }) {
	let runtime;
		...
		provider: {
			...
			async start(request) {
				...
				try {
					...
					const getTokens = scheduler === void 0 ? await createSubagentTokens({
						resolveAuth,
						store,
						refresh,
						signal
						...
						signal,
						model: thread.model,
						sessionId: request.parent.session.id,
						operation: (account) => {
...
function createRuntimeManagement({ manager, inspect, active, selectDsh }) {
	let operation;
	let state = {
		phase: "idle",
	...
	const start = async (action) => {
		...
		current.done = (async () => {
			try {
				...
				const result = action === "install" ? await host.installBundle(`${SUBAGENT_RUNTIME_PACKAGE}@${SUBAGENT_RUNTIME_VERSION}`, {
					enabled: false,
					requestId
...
function normalizeScheduler(value = {}) {
	const fixedAccountId = typeof value?.fixedAccountId === "string" && value.fixedAccountId.length > 0 ? value.fixedAccountId : void 0;
	return {
		strategy: SCHEDULER_STRATEGIES.has(value?.strategy) ? value.strategy : "fill-first",
		sessionAffinity: value?.sessionAffinity !== false,
		...fixedAccountId === void 0 ? {} : { fixedAccountId }
	};
}
...
function decodeJwtPayload$2(access) {
...
function emailFromAccessToken(access) {
	const payload = decodeJwtPayload$2(access);
...
function sanitizeOAuthCredential(value) {
	...
	const tokenAccountId = decodeJwtPayload$2(credential.access)?.["https://api.openai.com/auth"]?.chatgpt_account_id;
...
function sameAccountIdentity(left, right) {
	const leftAccountId = typeof left?.accountId === "string" && left.accountId.length > 0 ? left.accountId : void 0;
	const rightAccountId = typeof right?.accountId === "string" && right.accountId.length > 0 ? right.accountId : void 0;
	if (leftAccountId !== void 0 && rightAccountId !== void 0) return leftAccountId === rightAccountId;
	...
	return leftEmail !== void 0 && rightEmail !== void 0 && leftEmail === rightEmail;
}
function sameOAuthCredential(left, right) {
	return left?.type === right?.type && left?.access === right?.access && left?.refresh === right?.refresh && left?.expires === right?.expires && left?.accountId === right?.accountId && normalizeAccountEmail(left?.email)?.toLowerCase() === normalizeAccountEmail(right?.email)?.toLowerCase();
}
function matchingAccountIndex(accounts, credential) {
	...
	return accounts.findIndex((account) => account.credential.access === credential.access || account.credential.refresh === credential.refresh);
}
...
var DshOAuthAccountVault = class {
	...
	async #ensurePayload() {
		...
		if (existing !== void 0) {
			...
			if (payload.accounts.find((account) => account.enabled !== false) === void 0) return payload;
	...
	readActive() {
		return this.#enqueue(async () => {
			...
			return active?.enabled === false ? void 0 : clone$1(active?.credential);
		});
	}
	read(id) {
		return this.#enqueue(async () => {
	...
	updateScheduler(patch = {}) {
		return this.#enqueue(async () => {
			...
			if (fixedAccountId !== void 0 && fixedAccountId !== null && (typeof fixedAccountId !== "string" || fixedAccountId.length === 0)) throw new Error("Invalid Codex fixed account");
			return normalizeScheduler((await this.#modifyPayload((current) => {
				if (typeof fixedAccountId === "string") {
					...
					if (account === void 0 || account.enabled === false) throw new Error("Invalid Codex fixed account");
				}
				const scheduler = normalizeScheduler({
					...current.scheduler,
					...patch,
					...fixedAccountId === null ? { fixedAccountId: void 0 } : {}
				});
				return {
					...current,
	...
	configure(id, patch = {}) {
		return this.#enqueue(async () => {
			...
			const payload = await this.#modifyPayload((current) => {
				...
				const activeId = accounts.find((account) => account.id === current.activeId)?.enabled === false ? accounts.find((account) => account.enabled !== false)?.id ?? current.activeId : current.activeId;
				const scheduler = patch.enabled === false && current.scheduler?.fixedAccountId === id ? normalizeScheduler({
					...current.scheduler,
					fixedAccountId: void 0
				}) : current.scheduler;
				return {
					...current,
					activeId,
	...
	importMany(entries) {
		return this.#enqueue(async () => {
			...
			const merge = (source) => {
				const accounts = source.map((account) => clone$1(account));
				let added = 0;
				let updated = 0;
				let duplicates = 0;
				for (const entry of validated) {
					...
					if (index >= 0) {
						...
						if (sameAccountIdentity(current.credential, entry.credential) && entry.credential.expires < current.credential.expires) {
							duplicates += 1;
							continue;
						}
						accounts[index] = {
	...
	select(id) {
		return this.#enqueue(async () => {
			await this.#modifyPayload((current) => {
				...
				if (account === void 0) throw new Error("Unknown Codex account");
		...
		return this.#enqueue(async () => {
			await this.#modifyPayload((current) => {
				...
				const next = {
					...
					accounts,
					scheduler: current.scheduler?.fixedAccountId === id ? normalizeScheduler({
						...current.scheduler,
						fixedAccountId: void 0
					}) : current.scheduler
				};
...
function credentialFromObject(raw) {
	...
	if (expires === void 0) {
		const exp = decodeJwtPayload$1(access)?.exp;
...
var CodexLoginCoordinator = class {
	...
	async updateScheduler(patch) {
		...
		if (patch.sessionAffinity !== void 0 && typeof patch.sessionAffinity !== "boolean") throw new Error("Invalid Codex scheduler affinity");
		if (patch.rollingActivation !== void 0 && typeof patch.rollingActivation !== "boolean") throw new Error("Invalid Codex rolling activation setting");
		if (Object.hasOwn(patch, "fixedAccountId") && patch.fixedAccountId !== null && (typeof patch.fixedAccountId !== "string" || patch.fixedAccountId.length === 0)) throw new Error("Invalid Codex fixed account");
	...
	return async (endpoint, payload, signal) => {
		try {
			...
			if (endpoint === "scheduler/update") return ok(await coordinator.updateScheduler({
				...
				...input.rollingActivation === void 0 ? {} : { rollingActivation: input.rollingActivation },
...
//#endregion
//#region src/codex-request.js
const itemId = (prefix, session, value) => `${prefix}_${createHash("sha256").update(JSON.stringify([session, value])).digest("hex").slice(0, 32)}`;
...
function officialCodexRequest(payload, metadata) {
	if (!metadata) return payload;
	const reasoning = { ...payload.reasoning };
	if (reasoning.effort === void 0 && metadata.defaultReasoningEffort) reasoning.effort = metadata.defaultReasoningEffort;
	if (metadata.supportsReasoningSummary === false || metadata.defaultReasoningSummary === "none") delete reasoning.summary;
	else if (metadata.defaultReasoningSummary !== void 0) reasoning.summary = metadata.defaultReasoningSummary;
	const next = {
		...payload,
		reasoning
	};
...
function quotaRetryAfterMs(payload) {
	...
	if (quota?.type !== "usage_limit_reached" && quota?.code !== "usage_limit_reached") return void 0;
	...
	const at = absolute < 1e10 ? absolute * 1e3 : absolute;
	return Math.max(1e3, at - Date.now());
}
/**
...
function openaiCodexSubscriptionProvider({ resolveSpeedMode = () => void 0, resolveOutputVerbosity = () => OUTPUT_VERBOSITY_DEFAULT, resolveContextMode = () => void 0, resolveCustomContextWindow = () => void 0, catalog, connection, compaction, runNetwork = (_area, operation) => operation() } = {}) {
	...
	const withPreferences = (model, options = {}) => {
		...
		return {
			...
			...metadata?.useResponsesLite === true ? { headers: {
				...options.headers,
	...
	const networkIterable = (factory, optionsFactory, requestedModel) => {
				...
				Promise.resolve().then(async () => {
					...
					running = Promise.resolve().then(() => runNetwork("model", async () => {
						...
						async transformResponse(response, target) {
							...
							if (!transformed.ok && target.hostname === "chatgpt.com" && target.pathname === "/backend-api/codex/responses") try {
								relayMetadata.retryAfterMs = quotaRetryAfterMs(await transformed.clone().json());
							} catch {}
		...
		const step = async (method, value) => {
			...
			try {
				const result = await scoped(() => iterator[method]?.(value) ?? (method === "throw" ? Promise.reject(value) : Promise.resolve({
					done: true,
					value
				})));
				...
				if (event?.type === "error" && relayMetadata.retryAfterMs > 0 && /usage limit|usage_limit_reached|quota/iu.test(event.error?.errorMessage ?? "")) event = {
					...event,
					error: {
						...event.error,
						errorMessage: `${event.error.errorMessage} resets_in_seconds: ${Math.ceil(relayMetadata.retryAfterMs / 1e3)}`
					}
				};
				return {
					...result,
		...
		return {
			...
			return: (value) => ready || iterator ? step("return", value) : Promise.resolve({
				done: true,
				value
			}),
...
function visibleModel(value) {
	...
	return {
		...
		...[
			...
			"concise",
			"detailed"
		].includes(value.default_reasoning_summary) ? { defaultReasoningSummary: value.default_reasoning_summary } : {},
		...nonEmpty$2(value.default_reasoning_level) ? { defaultReasoningEffort: value.default_reasoning_level } : {},
	...
	const refresh = ({ signal, modelId } = {}) => {
		...
		const work = (async () => {
			const request = async () => {
				...
				if (typeof access !== "string" || access.length === 0 || typeof accountId !== "string" || accountId.length === 0) throw new Error("Codex model catalog has no usable account credential");
				const headers = {
					...
					...etag === void 0 || modelId !== void 0 ? {} : { "if-none-match": etag }
				};
				return fetchCatalog(CODEX_MODELS_URL, {
					method: "GET",
					redirect: "error",
					headers,
			...
			const accountIds = options.accountIds === void 0 ? void 0 : await options.accountIds();
			let response;
			let requiredModels;
			let lastError;
			for (const id of Array.isArray(accountIds) && accountIds.length > 0 ? accountIds : [void 0]) {
				requestSignal.throwIfAborted();
				try {
					response = id === void 0 ? await request() : await options.withAccount(id, request);
					if (response.status === 304) break;
					if (response.ok && modelId !== void 0) {
						requiredModels = parseOfficialModelCatalog(await response.clone().json());
						if (requiredModels.some((model) => model.id === modelId)) break;
						lastError = /* @__PURE__ */ new Error(`Codex account does not advertise model ${modelId}`);
						response = void 0;
						continue;
					}
					if (response.ok) break;
					lastError = /* @__PURE__ */ new Error(`Codex model catalog failed (HTTP ${response.status})`);
					response = void 0;
				} catch (error) {
					lastError = error;
				}
			}
			if (response === void 0) throw lastError ?? /* @__PURE__ */ new Error("Codex model catalog is unavailable");
...
async function readJsonWithin(response, maximumBytes) {
	...
	if (response.body === null) throw Object.assign(/* @__PURE__ */ new Error("Codex returned an unreadable image response"), { code: "CODEX_IMAGE_INCOMPLETE" });
	...
	try {
		...
		throw Object.assign(/* @__PURE__ */ new Error("Codex returned an unreadable image response"), { code: "CODEX_IMAGE_INCOMPLETE" });
	}
}
...
function responseMetadata(value) {
	...
	if (typeof encoded !== "string") throw Object.assign(/* @__PURE__ */ new Error("Codex returned no image data"), { code: "CODEX_IMAGE_INCOMPLETE" });
	return {
		encoded,
...
function createCodexImageTool(options) {
	...
	return defineTool({
		...
		async execute(args, exec) {
			...
			if (metadata.reportedModel !== void 0 && metadata.reportedModel !== resolveImageModel(args.model)) throw Object.assign(/* @__PURE__ */ new Error(`Upstream model mismatch: requested ${resolveImageModel(args.model)}, received ${metadata.reportedModel}`), { code: "CODEX_MODEL_MISMATCH" });
...
//#endregion
//#region src/tool-step-recovery.js
const TRANSIENT_TOOL_CODES = /* @__PURE__ */ new Set([
	"WEB_PROVIDER_ERROR",
	"WEB_FETCH_TIMEOUT",
	"TOOL_TIMEOUT",
	"TRANSPORT",
...
const REQUEST_TOOL_CODES = /* @__PURE__ */ new Set([
	"WEB_INVALID_URL",
	"WEB_BLOCKED_URL",
...
function isSafeShellQuery(command) {
	if (typeof command !== "string" || /[;&|<>`$\\\r\n]/u.test(command)) return false;
	const value = command.trim();
	return value === "pwd" || value === "pwd -P" || /^git status(?: --short)?$/u.test(value) || /^(?:ls|cat)\s+(?!-)[\p{L}\p{N}._/ -]+$/u.test(value) || /^rg\s+(?!-)[\p{L}\p{N}._/-]+(?:\s+(?!-)[\p{L}\p{N}._/-]+)?$/u.test(value);
}
function readOnlyTool(exec) {
	if ([
		...
		"web_fetch",
		"read"
	].includes(exec.name)) return true;
	return exec.name === "bash" && exec.arguments?.run_in_background !== true && isSafeShellQuery(exec.arguments?.command);
}
function retryableResult(exec, result) {
	...
	return exec.name === "bash" && result?.value?.kind === "foreground" && (result.value.timedOut === true || result.value.aborted === true);
}
...
function runtimeScheduler(tools) {
	...
	const candidates = Object.getOwnPropertySymbols(tools).filter((key) => key.description === TOOL_RUNTIME_SCHEDULER.description).map((key) => tools[key]).filter((value) => typeof value?.dispatch === "function");
	return candidates.length === 1 ? candidates[0] : void 0;
}
/** Retry only incomplete, read-only tool attempts before DSH commits a result. */
async function recoverToolStep(exec, next, retryDispatch, onRetry = () => {}) {
	if (exec.agent?.session?.requestContext?.()?.provider !== "openai-codex" || !readOnlyTool(exec)) return next();
	const parentSignal = exec.signal;
	exec.signal = AbortSignal.any([parentSignal, AbortSignal.timeout(9e4)]);
	try {
		let result = await next();
		for (let attempt = 1; attempt < 3; attempt++) {
			if (exec.signal.aborted || !retryableResult(exec, result) || retryDispatch === void 0) return result;
...
function registerToolStepRecovery(ctx) {
	const retrying = /* @__PURE__ */ new WeakSet();
	return ctx.on?.("tools/execute", (exec, next) => {
		if (retrying.has(exec)) return next();
		const scheduler = runtimeScheduler(ctx.tools);
		return recoverToolStep(exec, next, typeof scheduler?.dispatch === "function" ? async () => {
			retrying.add(exec);
			try {
				return (await scheduler.dispatch(exec)).result;
			} finally {
				retrying.delete(exec);
			}
		} : void 0, (name, attempt) => ctx.logger?.debug?.("Codex read-only tool %s retry %d", name, attempt));
	});
}
//#endregion
//#region src/account-usage.js
const WEEK_SECONDS$1 = 604800;
...
const reusedRefreshToken = (error) => {
	for (let cause = error, depth = 0; cause && depth < 5; cause = cause.cause, depth += 1) if (typeof cause.message === "string" && /refresh_token_reused/u.test(cause.message)) return true;
	return false;
};
const publicError$1 = (error) => PUBLIC_USAGE_ERRORS.has(error?.message) ? error.message : reusedRefreshToken(error) ? "ChatGPT sign-in needs to be renewed" : "Could not read ChatGPT usage";
...
function planWeight(access) {
	try {
		const plan = JSON.parse(Buffer.from(access.split(".")[1], "base64url").toString("utf8"))?.["https://api.openai.com/auth"]?.chatgpt_plan_type?.toLowerCase?.();
		...
		].includes(plan)) return 5;
		if (plan === "pro") return 20;
	} catch {}
}
function weeklyQuotaPressure(usage, now = Date.now()) {
	const window = (usage?.rateLimits?.find((limit) => limit.id === "codex")?.windows ?? []).find((item) => Number.isFinite(item.windowSeconds) && Math.abs(item.windowSeconds - WEEK_SECONDS$1) <= WEEK_SECONDS$1 * .05);
	if (window === void 0 || !Number.isFinite(window.remainingPercent) || !Number.isSafeInteger(window.resetsAt)) return void 0;
	const hours = (window.resetsAt * 1e3 - now) / 36e5;
	if (!Number.isFinite(hours) || hours <= 0) return void 0;
	return window.remainingPercent / hours;
}
function quotaWindow$1(usage, seconds) {
	const window = (usage?.rateLimits?.find((limit) => limit.id === "codex")?.windows ?? []).find((item) => Number.isFinite(item.windowSeconds) && Math.abs(item.windowSeconds - seconds) <= seconds * .05);
	return window === void 0 ? void 0 : {
		remaining: window.remainingPercent,
		resetAt: Number.isSafeInteger(window.resetsAt) ? window.resetsAt * 1e3 : null
	};
}
function aggregateWindow(rows, kind) {
	const values = rows.map((row) => row.windows?.[kind] && {
		...row.windows[kind],
		weight: row.weight
	}).filter(Boolean);
...
function createAccountUsageService({ accountVault, store, createReader }) {
	...
	const readAccount = (account, force, signal) => store.withAccount(account.id, () => readerFor(account.id).read({
		force,
		signal
	...
	return Object.freeze({
		async readAll({ force = false, signal } = {}) {
			return {
				accounts: await mapAccounts(await accountVault?.list?.() ?? [], signal, async (account) => {
					...
					try {
						...
						return {
							id: account.id,
							usage,
							quotaPressure: weeklyQuotaPressure(usage)
						};
					} catch (error) {
						if (signal?.aborted) throw error;
						return {
		...
		async readPool({ force = false, signal } = {}) {
			...
			const rows = await mapAccounts(accounts, signal, async (account) => {
				try {
					const weight = planWeight((await store.withAccount(account.id, () => store.read("openai-codex", { signal })))?.access);
					if (weight === void 0) throw new Error("Unknown ChatGPT plan capacity");
					const usage = await readAccount(account, force, signal);
					return {
						weight,
						windows: {
							"5h": quotaWindow$1(usage, 18e3),
							week: quotaWindow$1(usage, WEEK_SECONDS$1)
						}
					};
				} catch (error) {
					if (signal?.aborted) throw error;
					return { error: publicError$1(error) };
				}
			});
			return {
				...
				accounts: rows,
				windows: {
					"5h": aggregateWindow(rows, "5h"),
					week: aggregateWindow(rows, "week")
				},
				fetchedAt: Date.now()
			};
		},
		...
		snapshot(id) {
			...
			return usage === void 0 ? void 0 : structuredClone(usage);
		},
		clear(id) {
			if (id === void 0) {
...
//#endregion
//#region src/quota-activation.js
const FIVE_HOUR_SECONDS = 18e3;
const WEEK_SECONDS = 604800;
const ACTIVATION_GRACE_MS = 2e3;
const RETRY_BASE_MS = 900 * 1e3;
const RETRY_MAX_MS = 3600 * 1e3;
...
function credentialPlanType(credential) {
	const plan = decodeJwtPayload(credential?.access)?.["https://api.openai.com/auth"]?.chatgpt_plan_type;
	return typeof plan === "string" ? plan.toLowerCase() : void 0;
}
function quotaWindow(usage, seconds) {
	return (usage?.rateLimits?.find((limit) => limit.id === "codex")?.windows ?? []).find((window) => Number.isFinite(window.windowSeconds) && Math.abs(window.windowSeconds - seconds) <= seconds * .05);
}
async function completeMinimalQuotaActivation(models, model, now = Date.now) {
...
function createQuotaActivationService({ listAccounts, readCredential, activate, readUsage, enabled = () => true, now = Date.now, setTimer = (callback, delay) => setTimeout(callback, delay), clearTimer = (handle) => clearTimeout(handle), onError = () => {} } = {}) {
	...
	const cancelTimer = (id) => {
		...
		if (handle !== void 0) clearTimer(handle);
		timers.delete(id);
	};
	const schedule = (id, at) => {
		cancelTimer(id);
		if (disposed || !enabled()) return;
		const delay = Math.max(1e3, at - now());
	...
	const eligible = async (id) => {
		...
		if (account === void 0 || account.enabled === false) return false;
		return credentialPlanType(await readCredential(id)) === "plus";
	};
	const nextFromUsage = (usage) => {
		const fiveHour = quotaWindow(usage, FIVE_HOUR_SECONDS);
		return Number.isSafeInteger(fiveHour?.resetsAt) && fiveHour.resetsAt * 1e3 > now() ? fiveHour.resetsAt * 1e3 + ACTIVATION_GRACE_MS : now() + FIVE_HOUR_SECONDS * 1e3 + ACTIVATION_GRACE_MS;
	};
	const run = async (id) => {
		...
		try {
			await activate(id);
			retries.delete(id);
			const row = await readUsage(id).catch(() => void 0);
			schedule(id, nextFromUsage(row?.usage ?? row));
		} catch (error) {
			const attempt = (retries.get(id) ?? 0) + 1;
			retries.set(id, attempt);
			const row = await readUsage(id).catch(() => void 0);
			const week = quotaWindow(row?.usage ?? row, WEEK_SECONDS);
			const next = Number(week?.remainingPercent) <= 0 && Number.isSafeInteger(week?.resetsAt) && week.resetsAt * 1e3 > now() ? week.resetsAt * 1e3 + ACTIVATION_GRACE_MS : now() + Math.min(RETRY_MAX_MS, RETRY_BASE_MS * 2 ** Math.min(attempt - 1, 2));
			schedule(id, next);
			onError(error, id);
		}
	};
	function enqueue(operation) {
		const current = tail.catch(() => void 0).then(operation);
		tail = current.catch(() => void 0);
		return current;
	}
	return Object.freeze({
		async sync() {
			...
			for (const account of accounts) {
				if (account.enabled === false) {
					cancelTimer(account.id);
					known.delete(account.id);
					retries.delete(account.id);
					continue;
				}
				if (credentialPlanType(await readCredential(account.id)) !== "plus") {
					cancelTimer(account.id);
					known.delete(account.id);
					retries.delete(account.id);
					continue;
...
function createSubscriptionRpcHandler({ authHandler, usageReader, accountUsageService, quotaActivator, resetCreditService, preferences, runtimeManagement, diagnosticsReader, modelCatalog, originalImages, resolveInheritedOriginal, closeConnections }) {
	return async (endpoint, payload, signal) => {
		if ([
			"runtime/status",
			"runtime/install",
		...
		if (endpoint === "usage/pool") try {
			signal.throwIfAborted();
			return {
				ok: true,
				value: await accountUsageService?.readPool({
					force: payload?.force === true,
					signal
				}) ?? {
					total: 0,
		...
		const fixedAccountChanged = endpoint === "scheduler/update" && Object.hasOwn(payload ?? {}, "fixedAccountId");
		if (result.ok === true && ([
			"logout",
			"account/select",
			"account/remove"
		].includes(endpoint) || fixedAccountChanged)) closeConnections?.();
		...
		if (endpoint === "logout" && result.ok === true) {
			...
			"account/remove",
			"account/configure"
		].includes(endpoint) || endpoint === "account/import" && (result.value?.added > 0 || result.value?.updated > 0) || fixedAccountChanged || endpoint === "login/status" && result.value?.authenticated === true)) {
		...
		if (result.ok === true && ([
			...
			"account/import",
			"account/configure"
		].includes(endpoint) || endpoint === "login/status" && result.value?.authenticated === true)) quotaActivator?.sync?.().catch(() => {});
		return result;
	};
}
//#endregion
...
const settingsFields = {
	...
	[SCHEDULER_STRATEGY_FIELD]: z.union([
		"fill-first",
		"round-robin",
		"weighted-round-robin",
		"quota-balanced"
	]).default(DEFAULT_SCHEDULER_STRATEGY),
	[SCHEDULER_SESSION_AFFINITY_FIELD]: z.boolean().default(true),
	[ACCOUNT_ROLLING_ACTIVATION_FIELD]: z.boolean().default(true),
...
function apply(ctx, config = {}) {
	...
	const schedulerConfig = async () => {
		...
		return {
			...
			...accountScheduler.fixedAccountId === void 0 ? {} : { fixedAccountId: accountScheduler.fixedAccountId }
		};
	};
	const scheduler = accountVault === void 0 ? void 0 : new CodexAccountScheduler(accountVault, {
	...
	const modelCatalog = createOfficialModelCatalog({
		...
		accountIds: async () => {
			...
			return accounts.filter((account) => account.enabled !== false).sort((left, right) => Number(right.id === active) - Number(left.id === active)).map((account) => account.id);
		},
	...
	ctx.effect(() => watchImageTool(settings, () => {
		...
		const scheduledImageTool = scheduler === void 0 ? imageTool : {
			...imageTool,
	...
	const scheduledAdapter = scheduler === void 0 ? compactedAdapter : new ScheduledCodexAdapter(compactedAdapter, scheduler, store, modelCatalog);
	...
	const codexSearch = scheduler === void 0 ? rawCodexSearch : Object.freeze({
		...rawCodexSearch,
	...
	const coordinator = new CodexLoginCoordinator(auth, {
		...
		updateSchedulerConfig: async (patch) => {
			if (Object.hasOwn(patch, "fixedAccountId")) await accountVault.updateScheduler({ fixedAccountId: patch.fixedAccountId });
			const settingsPatch = {
				...
				...patch.rollingActivation === void 0 ? {} : { [ACCOUNT_ROLLING_ACTIVATION_FIELD]: patch.rollingActivation }
			};
	...
	accountUsageService = accountVault === void 0 ? void 0 : createAccountUsageService({
		accountVault,
		store,
	...
	if (accountUsageService !== void 0 && normalizeSchedulerStrategy(settings.get()["accountSchedulerStrategy"]) === "quota-balanced") accountUsageService.readAll().catch((error) => ctx.logger?.debug?.("could not warm Codex account quota pressure: %s", error.message));
	...
	const quotaActivator = accountVault === void 0 ? void 0 : createQuotaActivationService({
		...
		activate: (id) => store.withAccount(id, async () => {
			...
			if (model === void 0) throw new Error("Codex Luna activation model is unavailable");
	...
	ctx.effect(() => {
		if (quotaActivator === void 0) return void 0;
		quotaActivator.sync();
		const unwatch = settings.watch(() => {
			quotaActivator.sync();
		});
		return () => {
			unwatch();
