...
function sanitizeOAuthCredential(value) {
	...
	const tokenAccountId = decodeJwtPayload$1(credential.access)?.["https://api.openai.com/auth"]?.chatgpt_account_id;
			...
			const rows = await mapAccounts(accounts, signal, async (account) => {
				try {
					...
					return {
						weight,
						windows: {
							"5h": quotaWindow(usage, 18e3),
							week: quotaWindow(usage, 604800)
						}
					};
				} catch (error) {
					if (signal?.aborted) throw error;
...
function createSubscriptionRpcHandler({ authHandler, usageReader, accountUsageService, resetCreditService, preferences, runtimeManagement, diagnosticsReader, modelCatalog, originalImages, resolveInheritedOriginal, closeConnections }) {
	return async (endpoint, payload, signal) => {
		if ([
			"runtime/status",
			"runtime/install",
