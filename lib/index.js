...
function createSubscriptionRpcHandler({ authHandler, usageReader, accountUsageService, resetCreditService, preferences, runtimeManagement, diagnosticsReader, modelCatalog, originalImages, resolveInheritedOriginal, closeConnections }) {
	return async (endpoint, payload, signal) => {
		...
		if (endpoint === "logout" && result.ok === true) {
			...
			"account/import",
			"account/configure"
		].includes(endpoint) || fixedAccountChanged || endpoint === "login/status" && result.value?.authenticated === true)) {
...
function apply(ctx, config = {}) {
	...
	const scheduledAdapter = scheduler === void 0 ? adapter : new ScheduledCodexAdapter(adapter, scheduler, store);
