...
function classifyFailure(failure = {}) {
	...
	if ([401, 403].includes(status) || /AUTH|UNAUTHORIZED|FORBIDDEN/.test(code) || /invalidated oauth token|invalid(?:ated)? oauth access token/iu.test(message)) return {
		retryable: true,
		reason: "auth",
		cooldownMs: 60 * 1e3
	};
	if ([
		...
		"MODEL_UNAVAILABLE",
		"MODEL_ACCESS_DENIED"
	].includes(code) || (status === 400 || status === 404) && /model.{0,80}(?:not found|not available|not supported|does not exist|do not have access)/iu.test(message)) return {
		retryable: true,
		reason: "model-access",
		cooldownMs: 60 * 1e3
	};
...
function apply(ctx, config = {}) {
	...
	const scheduledAdapter = scheduler === void 0 ? adapter : new ScheduledCodexAdapter(adapter, scheduler, store, modelCatalog);
