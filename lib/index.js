...
const CODEX_USER_AGENT = `${CODEX_ORIGINATOR}/0.155.1`;
...
function createScopedWebSocketConstructor(WebSocketImpl = WebSocket) {
	return class CodexScopedWebSocket extends WebSocketImpl {
		constructor(url, protocolsOrOptions) {
			...
			this.codexSessionId = isCodex ? String(webSocketOptions.headers?.["session-id"] ?? webSocketOptions.headers?.["Session-Id"] ?? randomUUID()) : void 0;
			this.codexThreadId = this.codexSessionId;
			this.codexWindowId = this.codexSessionId === void 0 ? void 0 : `${this.codexThreadId}:0`;
			if (isCodex && typeof this.addEventListener === "function") this.addEventListener("message", (event) => {
				const info = safeModelEvent(event?.data);
				if (info !== void 0) this.codexRequestScope?.options?.onModelEvent?.(info);
			});
		}
		send(...args) {
			...
			if (raw === void 0) return super.send(...args);
			try {
				...
				scope?.options?.onModelRequest?.({
					...
					serviceTier: typeof request.service_tier === "string" ? request.service_tier : void 0,
	...
	if (activeScopes === 0) {
		...
		scopedFetch = async (input, init) => {
			...
			if (target.hostname === CODEX_SUBSCRIPTION_HOST && target.pathname === "/backend-api/codex/responses") {
				...
				nextInit = {
					...init,
					headers: officialCodexHeaders(init?.headers)
				};
			}
			...
			return route.url === void 0 ? baseFetch(input, nextInit) : proxyFetch(input, nextInit, route.url);
		};
...
function createCodexNetworkTransport(options = {}) {
	...
	const run = async (area, operation) => {
		...
		const transportFields = () => ({
			...
			...clientIdentity === void 0 ? {} : { clientIdentity },
			...requestedModel === void 0 ? {} : { requestedModel },
			...requestedServiceTier === void 0 ? {} : { requestedServiceTier },
			...serverModel === void 0 ? {} : { serverModel },
			...serverServiceTier === void 0 ? {} : { serverServiceTier },
			...firstEventMs === void 0 ? {} : { firstEventMs },
			...firstTextMs === void 0 ? {} : { firstTextMs },
			...outputTokens === void 0 ? {} : { outputTokens },
			...reasoningTokens === void 0 ? {} : { reasoningTokens },
			...reasoningEffort === void 0 ? {} : { reasoningEffort },
			...responsesLite === void 0 ? {} : { responsesLite },
		...
		try {
			const value = await withCodexNetwork(operation, {
				...
				onModelEvent: (info) => {
					...
					if (firstEventMs === void 0) firstEventMs = elapsed;
					if (info?.type === "response.output_text.delta" && firstTextMs === void 0) firstTextMs = elapsed;
			...
			else if (providerFailed) attempts.set(area, {
				status: "failed",
				stage: "provider",
				code: "provider-error",
				route,
		...
		const start = () => {
			...
			ready = new Promise((resolve, reject) => {
				running = Promise.resolve().then(() => runNetwork("model", async () => {
					...
					resolve({
						iterator: (await factory())[Symbol.asyncIterator](),
						scoped: AsyncLocalStorage.snapshot()
					});
					await completed;
				})).catch(reject);
			});
...
const clientIdentities = /* @__PURE__ */ new Set(["codex_cli_rs"]);
...
function safeRequests(network) {
	...
	for (const [area, value] of Object.entries(raw)) {
		...
		result[area] = {
			...
			...Number.isSafeInteger(value.firstEventMs) && value.firstEventMs >= 0 && value.firstEventMs <= 36e5 ? { firstEventMs: value.firstEventMs } : {},
			...Number.isSafeInteger(value.firstTextMs) && value.firstTextMs >= 0 && value.firstTextMs <= 36e5 ? { firstTextMs: value.firstTextMs } : {},
			...Number.isSafeInteger(value.outputTokens) && value.outputTokens >= 0 && value.outputTokens <= 1e7 ? { outputTokens: value.outputTokens } : {},
			...Number.isSafeInteger(value.reasoningTokens) && value.reasoningTokens >= 0 && value.reasoningTokens <= 1e7 ? { reasoningTokens: value.reasoningTokens } : {},
			...[
				"none",
				"minimal",
				"low",
				...
				"max",
				"ultra"
			].includes(value.reasoningEffort) ? { reasoningEffort: value.reasoningEffort } : {},
			...
			...Number.isSafeInteger(value.durationMs) && value.durationMs >= 0 && value.durationMs <= 36e5 ? { durationMs: value.durationMs } : {},
