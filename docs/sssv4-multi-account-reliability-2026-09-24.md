...
正式插件仓库最新 `pnpm check`：704 项测试，678 通过、0 失败、26 项按平台条件跳过；服务端/client 构建与 `2.1.35` 发行包打包通过。该数字代表插件仓库测试；安装包入口另需在正式 Runtime 中复验。
...
| Provider | 429 overload、500/502/503/504/520–526、HTTP 200 流内 selected-model capacity | 跨账号尝试；全局暂态最多退避 30 秒，不逐个污染账号 | HTTP 矩阵、CPA 型流内容量、跨轮恢复 |
...
1. 空 `block-start` 与内部 reasoning 过早提交，HTTP 200 流内失败无法接力。相同的假上游测试直接指向正式安装中的 `2.1.33/lib/index.js` 时失败，返回空 block 后的终局错误；指向新构建的 `2.1.34/lib/index.js` 时通过，A 自动接给 B。现在完整模型响应未成功前不向 DSH 提交文本或工具调用。正文中途、工具参数构造中故障也可安全重试，但首字可见时间变晚。
...
7. DSH 对工具异常只提交 `isError` 并进入下个模型步骤，没有自动保证重做。新增 `tools/execute` 钩子，限定在 Codex 发起的只读工具与保守 Shell 查询，完整成功前最多重试三次。发现 Cordis `next()` 是单次消费的流水线，直接二次调用会跳过后续策略钩子；回归测试先复现后，重试改为重新进入 DSH dispatch scheduler，每次都经过完整的 `tools/execute` 包装链，只在最终提交一次工具结果。**安装包验收又发现**宿主与插件各自加载的 `dsh-tools` 私有 Symbol 不同，原钩子会静默停止重试；双实例回归测试先在旧实现失败，现按宿主 `ToolRuntime` 上唯一的 scheduler Symbol 描述定位，并在正式包中复验。写/Git/消息不进入该路径。
...
- 工具只读重试最多三次，Provider/传输跨轮重试最多等待 30 秒；更长的全局故障、所有账号持续冷却时会诊断失败。固定诊断账号模式按设置不接力。
...
- 自动化运行的假上游覆盖了协议与插件真实入口。产品 Harness 支持 `CODEX_TEST_PLUGIN_ENTRY=<正式 Runtime 的 lib/index.js 绝对路径>`，可在物化后针对实际安装包重跑；真实账号 smoke、浏览器视觉 UI、正式 Runtime 加载与重启须在版本物化后单独记录，不能从本报告的模拟测试推出已上线。
