...
隔离检出的最新全套 `node --test tests/*.test.mjs`：704 项测试，678 通过、0 失败、26 项按平台条件跳过；同一检出的服务端/client 构建通过，正式插件仓库此前 `pnpm check` 的 `2.1.34` 发行包打包通过。该数字代表插件仓库测试，不代表正式 DSH 已加载该版。
